...
* Using a local server (such as VS Code's Liv Server), open the **index.html** file. Remember, homeworks will be graded using Chrome and VS Code's Live Server plugin.
...
As the coding final step, make the chart interactive. When the user updates the country value in the drop down, the chart should be redrawn to show the values for the newly selected country. Once you are finished with Step 7, your chart should look similar to the screenshot above.

### Step 8:

The final step will be to do a data abstraction on the `males_data.csv` dataset. In `index.html`, there is a `dataAbstractionSection` paragraph containing a list with three bullet points.

* For the first bullet point, add the dataset type, its availability, and its size.
* For the second bullet point, do an attribute abstraction on the `Year` attribute (type and cardinality/range is required).
* For the third bullet point, pick one of the countries you selected in Step 3 (replace the {Attribute} tag with its name}), and do an attribute abstraction on it (type, range/cardinality required).

You can reference the Data Abstraction lecture for examples of data abstractions and definitions. Once you finish the data abstraction, you are done! Be sure to commit and push your completed code by the deadline.
...
- Doing an incorrect animation will not receive any E.C.

---

## Grading

This assignment is worth 10 points. Steps 1-7 are worth 7 points, and Step 8 is worth 3 points.
# d3-lolli
